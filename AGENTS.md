...
- When providing a clickable link to `VoiceHelper.exe` for manual verification, also provide current context-fill information and a recommendation on whether to continue in the current chat or move to a new chat. Base the recommendation on the expected complexity of the next task:
