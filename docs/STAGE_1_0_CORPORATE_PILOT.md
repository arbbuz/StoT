...
GitHub Actions artifact остается code-only. Whisper модель `models\*.bin` не включается в архив и не попадает в git.

Для работы распознавания пользователь или администратор вручную копирует `ggml-small-q5_1.bin` в папку `models` рядом с `Dicta.exe`:

```text
models\ggml-small-q5_1.bin
```

Если модель добавлена после проверки поставки, `verify_dicta_package.ps1` может показать предупреждение о файлах, которых нет в `SHA256SUMS.txt`. Это нормально для вручную добавленной модели. Проверка хэшей code-only файлов при этом остается действительной.
