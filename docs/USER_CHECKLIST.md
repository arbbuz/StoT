...
3. Если это GitHub Actions code-only artifact, вручную скопировать `ggml-small-q5_1.bin` в папку `models` рядом с `Dicta.exe`.
...
7. Backend оставить "Авто", если не нужно вручную проверять Vulkan/CUDA/OpenVINO/AVX2/Compat.
...
- наличие локальной модели `ggml-small-q5_1.bin`;
