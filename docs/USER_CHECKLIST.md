...
2. Если это GitHub Actions code-only artifact, вручную скопировать модели в папку `models` рядом с `VoiceHelper.exe`.
3. Запускать именно `VoiceHelper.exe` из этой папки.
4. Не переносить один EXE отдельно от папок `_internal`, `models`, `.tools`, `assets`, `docs`, `scripts`.
5. При первом запуске разрешить Windows доступ к микрофону, если появится системный запрос.
6. При необходимости открыть "Настройки" -> "Производительность" и выбрать модель:
   - `tiny-q5_1` - для слабого ПК;
   - `base-q5_1` - обычный рекомендуемый вариант;
   - `small-q5_1` - точнее, но медленнее.
7. Там же при необходимости выбрать профиль скорости:
   - "Авто" - выбрать модель по бенчмарку;
   - "Быстро" - использовать `tiny-q5_1`;
   - "Баланс" - использовать `base-q5_1`;
   - "Точно" - использовать `small-q5_1`.
8. Backend оставить "Авто", если не нужно вручную проверять Vulkan/CUDA/OpenVINO/AVX2/Compat.
...
10. Нажать "Скопировать" или заранее включить "Автокопия", чтобы текст копировался после распознавания.
